...
class Map_Renderer:
    def __init__(self, game, config):
        ...
        self.central_location = (
            config["central_location"]["x"], config["central_location"]["y"])
        self.scale = config["scale"]
        self.img = Image.new(
            'RGB', (self.width, self.height), color=(0xea, 0xdd, 0xcd))
    ...
    def transform_location(self, location):
        x = (location[0] - self.central_location[0]) / \
            self.scale + self.width / 2
        y = (location[1] - self.central_location[1]) / \
    ...
    def render_ship(self, ship):
        ...
        draw.ellipse((location[0] - radius, location[1] - radius,
                     location[0] + radius, location[1] + radius), fill=color)
